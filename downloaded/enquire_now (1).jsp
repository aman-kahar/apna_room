...
%>
 ...
 <%
	...
	Class.forName("com.mysql.jdbc.Driver").newInstance();
  conn2 = DriverManager.getConnection("jdbc:mysql://localhost:3306/apnaroom","root","");
	stmt2 = conn2.createStatement();
  rs2 = stmt2.executeQuery("select * from sign_up_page where user_id = " +(String)session.getAttribute("user_id"));
 %>
 ...
 while(rs2.next()){
 %>
 <input type="hidden" id="customer_name" name="customer_name" value="<%=rs2.getString("u_name")%>">
 <input type="hidden" id="contact" name="contact" value="<%=rs2.getString("mobile")%>">
 <input type="hidden" id="email_id" name="email_id" value="<%=rs2.getString("u_email")%>">
 <input type="hidden" id="ownwer_id" name="owner_id" value="<%=userId%>">
 <button type="Submit" value="VISIT ROOM ">VISIT ROOM </button>
 </form>
 <%
 }
 rs1.close();
